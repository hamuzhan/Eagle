...
Eagle: NVIDIA's open-source family of vision-language model projects, including the original Eagle VLMs, Eagle 2.5 long-context image/video VLMs, and LocateAnything grounding models.

This is a Python/PyTorch research codebase with training, inference, evaluation, Streamlit/Gradio demos, and TensorRT-LLM deployment utilities. Most meaningful runs require NVIDIA GPUs, model checkpoints, and external datasets.
...
- Run formatting before submitting code: `black .`, `isort .`, and `flake8 .` from the relevant subproject or repo root, depending on the changed files.
- Do not check in model weights, datasets, generated checkpoints, `work_dirs/`, logs, W&B artifacts, LMDB/parquet data outputs, TensorRT engines, or Hugging Face cache contents.
- Confirm before starting long-running or expensive work such as multi-GPU training, full evaluation, Docker builds, TensorRT-LLM builds, MagiAttention builds, or large dataset downloads.
- `Eagle2_5/` and `Embodied/` both define a top-level Python package named `eaglevl`. Do not install both into the same editable environment unless you intentionally want one to shadow the other.
...
Run commands from the directory shown in the command unless noted otherwise.

| Task | Command |
|------|---------|
| Format Python | `black . && isort . && flake8 .` |
| Original Eagle install | `cd Eagle && pip install -r requirements.txt && pip install .` |
| Original Eagle inference | `cd Eagle && python predict_demo.py` |
| Original Eagle Gradio demo | `cd Eagle && python gradio_demo.py --model-path ${MODEL_CKPT} --conv-mode vicuna_v1` |
| Original Eagle LMMs-Eval suite | `cd Eagle && bash scripts/eval_lmms_eval/eval-mme-seed-pope-sqa-gqa-ocrbench-textvqa-chartqa.sh $REPO_ID_OR_LOCAL_PATH $MODEL_NAME $CONV_MODE` |
| Eagle 2.5 install | `cd Eagle2_5 && pip install -e .` |
| Eagle 2.5 prepare data | `cd Eagle2_5 && bash shell/prepare.sh playground/sft_recipe/debug.json` |
| Eagle 2.5 finetune | `cd Eagle2_5 && GPUS=8 bash shell/train_stage2.sh 1 work_dirs/eagle2.5_debug` |
| Eagle 2.5 Streamlit demo | `cd Eagle2_5/streamlit_demo && bash start_demo.sh` |
| LocateAnything install | `cd Embodied && pip install -e .` |
| LocateAnything finetune | `cd Embodied && torchrun --nproc_per_node=8 eaglevl/train/locany_finetune_magi_stream.py --model_name_or_path nvidia/LocateAnything-3B --meta_path ./locany_recipe/my_recipe.json --output_dir work_dirs/my_sft --attn_implementation magi --deepspeed deepspeed_configs/zero_stage2_config.json` |
| LocateAnything COCO eval | `cd Embodied && bash evaluation/scripts/eval_coco.sh --model_path path/to/model --test_jsonl path/to/EvalData/_annotations/box_eval/COCO.jsonl --image_root path/to/EvalData --coco_json path/to/EvalData/coco/instances_val2017.json --output_dir path/to/output` |
| LocateAnything grounding eval | `cd Embodied && bash evaluation/scripts/eval_grounding.sh --dataset Dense200 --eval_type box_eval --model_path path/to/model --image_root path/to/EvalData --output_base path/to/output_base` |
...
- `flash-attn` installation must match the local CUDA/PyTorch stack; do not upgrade PyTorch or Transformers casually because versions are tightly coupled.
...
```text
playground/sft_recipe/*.json + JSONL/media -> shell/prepare.sh
    -> .prepare.json/parquet metadata -> shell/train_stage2.sh
...
Coordinates are normalized integers in `[0, 1000]`; divide by 1000 and multiply by image width/height for pixel coordinates.

## Design Patterns

| Pattern | Key Points |
|---------|------------|
| Hugging Face model integration | Custom configs inherit `PretrainedConfig`; models inherit `PreTrainedModel` and often `GenerationMixin`; `trust_remote_code=True` is expected for released checkpoints. |
...
- Do not alter prompt templates, special tokens, coordinate token formats, or output formats without updating training data, inference parsing, and evaluation code.
...
- Do not launch training scripts with placeholder paths. Replace `PATH_TO_*`, `playground/...`, `locany_recipe/...`, and `path/to/...` values first.
...
- Do not remove `trust_remote_code=True` from examples unless you verified released checkpoints still load.
...
|-------------|----------------------|
| Docs only | Review rendered Markdown and run `git diff --check`. |
| Formatting-only Python changes | `black . && isort . && flake8 .` in the relevant subproject. |
| Original Eagle inference/model loading | `cd Eagle && python predict_demo.py` when GPU and checkpoint access are available. |
| Original Eagle eval code | `cd Eagle && bash scripts/eval_lmms_eval/...` with the intended task script and datasets. |
| Eagle 2.5 data prep | `cd Eagle2_5 && bash shell/prepare.sh <recipe>` on a small/debug recipe. |
| Eagle 2.5 training code | `cd Eagle2_5 && GPUS=<n> bash shell/train_stage2.sh <nodes> <output_dir>` on a small/debug run. |
| Eagle 2.5 inference | Follow `Eagle2_5/document/5.inference.md` with `nvidia/Eagle-2.5-8B` or a local checkpoint. |
| Eagle 2.5 deployment | Follow `Eagle2_5/deployment/README.md`; requires Docker/TensorRT/TensorRT-LLM and patched TensorRT-LLM commit. |
| LocateAnything worker | Run a small `LocateAnythingWorker` inference on one image when GPU/model access is available. |
| LocateAnything training | Run a short `torchrun` smoke test with a tiny recipe and appropriate attention backend. |
| LocateAnything evaluation | Use scripts under `Embodied/evaluation/scripts/` with prepared EvalData. |
